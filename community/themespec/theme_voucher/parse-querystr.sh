# query_string="?fas=, complete=true, voucher=(207) 440 - 8296, email=jacob@gofocus.space, zipcode=234324, tos=accepted"
# query_string="?fas="

query_string="fas=fsdkljfiaoji8227238,complete=true,voucher=(207) 440 - 8296,email=jacob@gofocus.space,zipcode=23423,tos=accepted"
# Remove the leading '?' if present
query_string="${query_string#\?}"

# Remove 'fas' and 'tos' from query string
cleaned_query=$(echo "$query_string" | sed -E 's/(fas=[^,]*,? *|tos=[^,]*,? *)//g')

# Parse and export variables
echo "$cleaned_query" | IFS=', ' read -r -a params

# Iterate through the parameters
IFS=',' 
for param in $cleaned_query; do
    key=$(echo "$param" | cut -d= -f1)
    value=$(echo "$param" | cut -d= -f2-)

    # Export key-value pair
    export "$key=$value"

    # Debugging output
    echo "$key=$value"
done

# Debug output (optional)
# echo "Cleaned Query: $test_query"
# echo "Email: $email"
# echo "Zipcode: $zipcode"
# echo "Voucher: $voucher"
# echo "Continue: $continue"

# # Remove the leading "?" if present
# # function urldecode() { : "${*//+/ }"; echo -e "${_//%/\\x}"; }
# cleaned_query=$(echo "$query_string" | sed -E 's/(fas=[^,]*, |tos=[^,]*, )//g')
# echo $decoded_query

# # Parse and bind variables dynamically
# # while IFS='=' read -r key value; do
# #     declare "$key=$value"
# # done < <(echo "$decoded_query" | tr '&' '\n')

# # Build the environment variable string dynamically
# while IFS='=' read -r key value; do
#     export "$key=$value"
# done < <(echo "$cleaned_query" | tr ', ' '\n')

# # Output the variables
# # echo "env_vars $env_vars"
# echo "Email=$email"

# # eval "env $env_vars cowsay 'moo'"